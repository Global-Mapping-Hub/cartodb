...
module FrontendConfigHelper
  ...
  def frontend_config_hash(user = current_user)
    ...
    if user.present?
      ...
      config[:bigquery_enabled] = Cartodb.get_config(:connectors, 'bigquery', 'enabled')
      ...
      config[:snowflake_enabled] = Cartodb.get_config(:connectors, 'snowflake', 'enabled')
      config[:redshift_enabled] = Cartodb.get_config(:connectors, 'redshift', 'enabled')
